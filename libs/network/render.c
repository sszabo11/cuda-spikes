...
int render_from_thread(data_mutex_t *obj) {
  Vector2 *cords =
      rand_cords(obj->config->n_neurons, 200, WIDTH - 200, 50, HEIGHT - 50);
  Vector2 *cords_input = rand_cords(inputs, 50, 200, 50, HEIGHT - 50);
  Vector2 *cords_output =
      rand_cords(outputs, WIDTH - 200, WIDTH - 50, 50, HEIGHT - 50);
  ...
  uint8_t *spikes = malloc(n_neurons * sizeof(uint8_t));
  while (!WindowShouldClose()) {

    pthread_mutex_lock(&obj->mutex);
    while (obj->frame_ready == 0) {
      pthread_cond_wait(&obj->compute_done, &obj->mutex);
    }

    memcpy(spikes, obj->front->post_spikes, n_neurons * sizeof(uint8_t));
    obj->frame_ready = 0;

    pthread_cond_signal(&obj->render_done);
