...
int main() {
  ...
  // Print digits - works
  for (int i = 0; i < 10; i++) {
    mnist_image_t *img = &test_dataset->images[i];
  ...
  for (int img_idx = 0; img_idx < 1000; img_idx++) {
    ...
    //   printf("st: %d\n", st->data[i]);
    // }
    compute(d_net, h_net, st, img_idx, true);
  ...
  test_images(d_net, h_net, test_dataset);
  // for (int img_idx = 0; img_idx < 100; img_idx++) {
  //   mnist_image_t *img = &train_dataset->images[img_idx];
  //   SpikeTrain *st = encode_mnist(img, T);
  //   compute(d_net, h_net, st, img_idx, false);

  //  int id = 0;
  //  int val = 0;
  //  for (int i = 0; i < h_net->config->n_neurons; i++) {
  //    uint8_t post = h_net->data->post_spikes[i];
  //    printf("post: %d\n", post);
  //  }
